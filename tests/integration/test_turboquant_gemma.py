...
Coverage (from Quant planning doc):
  1. update_and_fetch_view_mode         – view mode returns TurboQuantKeysView
  2. state_roundtrip                    – state dict round-trips without loss
  3. block_iterator_covers_all_tokens   – iter_rotated_kv_blocks yields every token
  4. gemma_attention_with_kvcache       – standard KVCache path in Attention
  5. gemma_attention_with_turboquant    – TurboQuantKCache path in Attention
  6. incremental_decode                 – multiple decode steps accumulate offset
  7. storage_breakdown_keys             – nbytes dominated by 3-bit codes + scales
"""
...
def _make_tq_cache(return_mode: str = "view") -> TurboQuantKCache:
