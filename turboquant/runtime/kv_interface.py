...
class KVCompressor:
    ...
    def update_and_fetch(
        ...
        try:
            ...
            # NaN guard — detect corrupted encode output before poisoning cache
            if mx.any(mx.isnan(ks)).item():  # type: ignore[union-attr]
                raise TurboQuantKernelError(
                    f"NaN detected in K scales after encode at offset "
                    f"{prev}\u2013{prev+T}. Numerical failure in quant pipeline."
                )

            # Store K
            self._k_packed[:, :, prev : prev + T, :] = pk  # type: ignore
            self._k_scales[:, :, prev : prev + T, :] = ks.astype(  # type: ignore
            ...
            # Restore offset so the caller sees a consistent state and can
            # choose to fall back to the original dense cache.
            self.offset = prev
            from turboquant.errors import CompressionFailureError  # noqa: PLC0415

            raise CompressionFailureError(
                f"KV compression failed at layer offset {prev}: {exc}"
            ) from exc
